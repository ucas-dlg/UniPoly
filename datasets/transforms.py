...
def crop_v2(image, target, region):
    ...
    crop_linestrings = []
    for linestring_pts in instance_pts:
        ...
        if translated_linestring.geom_type == 'LineString':
            ...
            for geom in translated_linestring.geoms:
                if geom.geom_type == 'LineString' or geom.geom_type == 'Polygon':
...
class linestring2pt(object):
    def __call__(self, image, target):
        ...
        for i in range(len(instances_pts)):
            ...
            if linestring_pts.length < 10 or len(linestring_pts.coords) > 64: continue  # 过滤掉过短或过长的线段
            if linestring_pts.is_ring:
                ...
                reorder_linestring_pts = reorder_linestring(linestring_pts, y_tolerance=2)  # 固定起点和方向
            # 插值到64个点
        ...
        # target['labels'] = torch.ones((len(new_pts),), dtype=torch.int64)  # 全部为1类
...
class linestring2ptV3(object):
    ...
    def __call__(self, image, target):
        ...
        for i in range(len(instances_pts)):
            linestring_pts = instances_pts[i]
            if len(set(list(linestring_pts.coords))) < 3 or len(linestring_pts.coords) > 511: continue  # 过滤掉过短或过长的线段
            if linestring_pts.is_ring:
                if is_clockwise(linestring_pts.coords):
                    linestring_pts = LineString(linestring_pts.coords[::-1])
            ...
            if (reorder_linestring_pts.bounds[3] - reorder_linestring_pts.bounds[1])  * (reorder_linestring_pts.bounds[2] - reorder_linestring_pts.bounds[0]) < 9 or reorder_linestring_pts.bounds[2]-reorder_linestring_pts.bounds[0]<3 or reorder_linestring_pts.bounds[3]-reorder_linestring_pts.bounds[1]<3: 
                continue
            # 插值到64个点
            pts, classes = interpolate_to_points_with_classes(reorder_linestring_pts.coords, target_points=512) 
            new_pts.append(pts)
            new_pts_classes.append(classes)
            new_bboxes.append(reorder_linestring_pts.bounds)
        ...
        if len(new_pts) == 0:
            target['instances_pts'] = torch.zeros((0, 512, 2), dtype=torch.float32)
            target['boxes'] = torch.zeros((0, 4), dtype=torch.float32)
            target['labels'] = torch.ones((len(new_pts),), dtype=torch.int64)
            target['area'] = torch.zeros((0,), dtype=torch.float32)
            target['coord_class'] = torch.zeros((0, 512), dtype=torch.float32)
...
class linestring2ptV4(object):
    ...
    def __call__(self, image, target):
        ...
        for i in range(len(instances_pts)):
            linestring_pts = instances_pts[i]
            if len(set(list(linestring_pts.coords))) < 3 or len(linestring_pts.coords) > 1023: continue  # 过滤掉过短或过长的线段
            if linestring_pts.is_ring:
                if is_clockwise(linestring_pts.coords):
                    linestring_pts = LineString(linestring_pts.coords[::-1])
                temp_polygon = Polygon(linestring_pts)
            ...
            if (reorder_linestring_pts.bounds[3] - reorder_linestring_pts.bounds[1])  * (reorder_linestring_pts.bounds[2] - reorder_linestring_pts.bounds[0]) < 9 or reorder_linestring_pts.bounds[2]-reorder_linestring_pts.bounds[0]<3 or reorder_linestring_pts.bounds[3]-reorder_linestring_pts.bounds[1]<3: 
                continue
            # 插值到64个点
            pts, classes = interpolate_to_points_with_classes(reorder_linestring_pts.coords, target_points=1024) 
            new_pts.append(pts)
            new_pts_classes.append(classes)
            new_bboxes.append(reorder_linestring_pts.bounds)
            new_bboxes_area.append((reorder_linestring_pts.bounds[3] - reorder_linestring_pts.bounds[1])  * (reorder_linestring_pts.bounds[2] - reorder_linestring_pts.bounds[0]))
        if len(new_pts) == 0:
            target['instances_pts'] = torch.zeros((0, 1024, 2), dtype=torch.float32)
            target['boxes'] = torch.zeros((0, 4), dtype=torch.float32)
            target['labels'] = torch.ones((len(new_pts),), dtype=torch.int64)
            target['area'] = torch.zeros((0,), dtype=torch.float32)
            target['coord_class'] = torch.zeros((0, 1024), dtype=torch.float32)
